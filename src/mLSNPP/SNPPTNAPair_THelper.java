package mLSNPP;


/**
* mLSNPP/SNPPTNAPair_THelper.java .
* Generated by the IDL-to-Java compiler (portable), version "3.2"
* from mLSNPP.idl
* Wednesday, June 15, 2016 7:24:38 PM COT
*/

abstract public class SNPPTNAPair_THelper
{
  private static String  _id = "IDL:mtnm.tmforum.org/mLSNPP/SNPPTNAPair_T:1.0";

  public static void insert (org.omg.CORBA.Any a, mLSNPP.SNPPTNAPair_T that)
  {
    org.omg.CORBA.portable.OutputStream out = a.create_output_stream ();
    a.type (type ());
    write (out, that);
    a.read_value (out.create_input_stream (), type ());
  }

  public static mLSNPP.SNPPTNAPair_T extract (org.omg.CORBA.Any a)
  {
    return read (a.create_input_stream ());
  }

  private static org.omg.CORBA.TypeCode __typeCode = null;
  private static boolean __active = false;
  synchronized public static org.omg.CORBA.TypeCode type ()
  {
    if (__typeCode == null)
    {
      synchronized (org.omg.CORBA.TypeCode.class)
      {
        if (__typeCode == null)
        {
          if (__active)
          {
            return org.omg.CORBA.ORB.init().create_recursive_tc ( _id );
          }
          __active = true;
          org.omg.CORBA.StructMember[] _members0 = new org.omg.CORBA.StructMember [5];
          org.omg.CORBA.TypeCode _tcOf_members0 = null;
          _tcOf_members0 = org.omg.CORBA.ORB.init ().create_string_tc (0);
          _members0[0] = new org.omg.CORBA.StructMember (
            "sNPPId",
            _tcOf_members0,
            null);
          _tcOf_members0 = org.omg.CORBA.ORB.init ().get_primitive_tc (org.omg.CORBA.TCKind.tk_short);
          _tcOf_members0 = org.omg.CORBA.ORB.init ().create_alias_tc (transmissionParameters.LayerRate_THelper.id (), "LayerRate_T", _tcOf_members0);
          _members0[1] = new org.omg.CORBA.StructMember (
            "layerRate",
            _tcOf_members0,
            null);
          _tcOf_members0 = org.omg.CORBA.ORB.init ().create_string_tc (0);
          _members0[2] = new org.omg.CORBA.StructMember (
            "rAId",
            _tcOf_members0,
            null);
          _tcOf_members0 = org.omg.CORBA.ORB.init ().create_string_tc (0);
          _tcOf_members0 = org.omg.CORBA.ORB.init ().create_alias_tc (mLSNPP.TNAName_THelper.id (), "TNAName_T", _tcOf_members0);
          _members0[3] = new org.omg.CORBA.StructMember (
            "tNAName",
            _tcOf_members0,
            null);
          _tcOf_members0 = org.omg.CORBA.ORB.init ().create_string_tc (0);
          _tcOf_members0 = org.omg.CORBA.ORB.init ().create_alias_tc (mLSNPP.TNAName_THelper.id (), "TNAName_T", _tcOf_members0);
          _members0[4] = new org.omg.CORBA.StructMember (
            "groupTNAName",
            _tcOf_members0,
            null);
          __typeCode = org.omg.CORBA.ORB.init ().create_struct_tc (mLSNPP.SNPPTNAPair_THelper.id (), "SNPPTNAPair_T", _members0);
          __active = false;
        }
      }
    }
    return __typeCode;
  }

  public static String id ()
  {
    return _id;
  }

  public static mLSNPP.SNPPTNAPair_T read (org.omg.CORBA.portable.InputStream istream)
  {
    mLSNPP.SNPPTNAPair_T value = new mLSNPP.SNPPTNAPair_T ();
    value.sNPPId = istream.read_string ();
    value.layerRate = istream.read_short ();
    value.rAId = istream.read_string ();
    value.tNAName = istream.read_string ();
    value.groupTNAName = istream.read_string ();
    return value;
  }

  public static void write (org.omg.CORBA.portable.OutputStream ostream, mLSNPP.SNPPTNAPair_T value)
  {
    ostream.write_string (value.sNPPId);
    ostream.write_short (value.layerRate);
    ostream.write_string (value.rAId);
    ostream.write_string (value.tNAName);
    ostream.write_string (value.groupTNAName);
  }

}
